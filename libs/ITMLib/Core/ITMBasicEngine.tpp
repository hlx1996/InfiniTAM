...
//	Vector2i trackedImageSize = trackingController->GetTrackedImageSize(imgSize_rgb, imgSize_d);
	...
	renderState_live = ITMRenderStateFactory<TIndex>::CreateRenderState(imgSize_d, scene->sceneParams, memoryType);
	renderState_freeview = NULL; //will be created if needed

	trackingState = new ITMTrackingState(imgSize_d, memoryType);
...
template<typename TVoxel, typename TIndex>
ITMScene<TVoxel, TIndex> *ITMBasicEngine<TVoxel, TIndex>::getScene() const {
	return scene;
}

template<typename TVoxel, typename TIndex>
ITMTrackingState *ITMBasicEngine<TVoxel, TIndex>::getTrackingState() const {
